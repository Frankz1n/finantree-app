    ...
    return (
        <div className="space-y-8 pb-24 md:pb-8">
            <header className="flex items-center justify-between">
                ...
                <Button className="h-12 w-12 rounded-full bg-slate-900 p-0 shadow-lg hover:bg-slate-800 transition-transform hover:scale-105">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {savingBoxes.map((box, index) => {
                    ...
                    return (
                        ...
                        >
                            <Card className="group relative overflow-hidden rounded-[32px] border-none bg-white p-6 shadow-sm hover:shadow-md transition-all duration-300">
                                <div className="flex justify-between items-start mb-8">
                                    ...
                                    <div className="bg-slate-50 px-3 py-1 rounded-full text-xs font-bold text-slate-500">
                                        {progress}%
                                    </div>
                                </div>

                                <div className="space-y-1 mb-6">
                                    <h3 className="text-lg font-bold text-slate-900 truncate" title={box.name}>{box.name}</h3>
                                    <p className="text-xs font-medium text-slate-400">
                                        {formatCurrency(box.current_amount)} <span className="text-slate-300">/</span> {formatCurrency(box.target_amount)}
                                    </p>
                                </div>
                                ...
                                <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                                    <div
                                        className="h-full bg-[#00C980] rounded-full transition-all duration-1000 ease-out"
                ...
                >
                    <button className="w-full h-full min-h-[220px] rounded-[32px] border-2 border-dashed border-slate-200 bg-slate-50/50 flex flex-col items-center justify-center gap-4 text-slate-400 hover:border-slate-300 hover:text-slate-500 hover:bg-slate-100/50 transition-all group">
