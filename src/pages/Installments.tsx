...
export default function Installments() {
    ...
    const { totalRemaining, monthlyCommitment, banks } = useMemo(() => {
        ...
        mockInstallments.forEach(item => {
            bankSet.add(item.banco);
            monthly += item.valorParcela;
            const remainingInstallments = item.totalParcelas - item.parcelaAtual + 1;
            remaining += remainingInstallments * item.valorParcela;
        });
    ...
    const filteredInstallments = useMemo(() => {
        if (activeTab === "all") return mockInstallments;
        return mockInstallments.filter(item => item.banco === activeTab);
    }, [activeTab]);
            ...
            <div className="space-y-4">
                <div className="grid gap-4">
                    {filteredInstallments.map((item) => {
                        const progressPercentage = (item.parcelaAtual / item.totalParcelas) * 100;
                        const isFinished = item.parcelaAtual >= item.totalParcelas;
